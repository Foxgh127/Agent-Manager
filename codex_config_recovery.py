...
def repair_config(
    *, expected_fingerprint: str, backup_id: str | None = None, reset: bool = False
    ...
    with core.SWITCH_OPERATION_LOCK, core.CONFIG_FILE_LOCK, core.RUNTIME_OVERLAY_LOCK:
        ...
        snapshot = core._capture_file_bytes(
            (core.CONFIG_FILE, core.RUNTIME_OVERLAY_FILE)
        )
        if snapshot.get(core.CONFIG_FILE) != (
            original if original_exists else None
        ):
        ...
        try:
            ...
            rollback = core._restore_file_bytes(snapshot)
