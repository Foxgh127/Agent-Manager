...
一个面向 Windows 的本地 AI 开发工作台：管理 Codex 账号与 API、组织模型和子代理、维护会话与配置。当前版本 **1.1.8**。
...
.github/          持续集成与发布工作流
dist/             可分发安装包（生成，不入库）
artifacts/        构建中间文件和本轮检查材料（生成，不入库）
```

详细模块责任、公共接口及参考方案见 [架构说明](docs/architecture.md)。

源码和长期说明只放在上述对应目录。一次性日志、调试预览和测试下载放入 `artifacts/` 的分类子目录；`scripts/clean.ps1 -Preview` 可预览清理范围。`release/` 是已退役的旧输出目录，正式构建统一使用 `dist/`。
