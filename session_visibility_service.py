...
import codex_session_visibility as engine
...
def _core():
    import agent_manager_core
    return agent_manager_core
    ...
    for action in actions:
        ...
        if kind == "sqlite_update":
            ...
            if not (engine.STATE_DB_PATTERN.fullmatch(target.name)
                    or (table == "local_thread_catalog"
                        and target == home / engine.DESKTOP_CATALOG_PATH)):
                raise engine.SessionVisibilityError("会话可见性数据库路径无效。")
            allowed_fields = {"model_provider"} if table == "local_thread_catalog" else _PATCH_FIELDS
            if (table not in {"threads", "local_thread_catalog"}
                    ...
                    or action.get("idColumn") not in {"id", "thread_id"}):
...
def _completion_message(completion, changed):
    ...
    return ("已完成部分历史会话同步；" if changed else "历史会话同步尚未完成；") + detail + "。可在会话可见性面板运行深度诊断；缺失目录暂仅诊断。"
...
def repair_current_provider(session_ids=None):
    ...
    with _operation() as core:
        ...
        completion = {"partial": bool(pending or unresolved or missing_catalog or catalog_unavailable or stopped_reason),
                      ...
                      "remainingActions": len(pending or ()), "catalogRecovery": "diagnostic_only_no_insert",
                      "followUpMode": "deep"}
