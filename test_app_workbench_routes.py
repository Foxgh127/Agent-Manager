...
class WorkbenchRouteTests(unittest.TestCase):
    ...
    def test_relay_key_deletion_defaults_local_and_requires_explicit_website_scope(self):
        ...
        with patch.object(app.recovery_coordinator,"before_account_delete",side_effect=guarded), patch.object(app.recovery_coordinator,"create",side_effect=lambda *_args: events.append("remote-backup") or {"id":"remote-point"}):
    ...
    def test_failed_backup_blocks_remote_key_deletion(self):
        delete=Mock()
        self.runtime.relay_portal=SimpleNamespace(delete_saved_key=delete)
        with patch.object(app.recovery_coordinator,"create",side_effect=app.core.ManagerError("backup failed")):
            with self.assertRaises(urllib.error.HTTPError) as failed:
                self.request("/api/relay-accounts/saved/keys/7",method="DELETE",payload={"scope":"website"})
            failed.exception.close()
        delete.assert_not_called()
    ...
    def test_usage_export_route_passes_server_filtered_document_to_download_writer(self):
        with patch.object(usage_export_service.core, "save_json_export_to_downloads", return_value={"path": "test-export.json"}) as save:
    ...
    def test_codex_config_routes_read_and_save_documents(self):
        ...
        claim = patch.object(codex_config_recovery, "claim_orphaned_overlay", return_value=False)
        claim.start()
    ...
    def test_root_ui_uses_lightweight_account_sync_without_whole_page_refresh(self):
        source = (Path(__file__).parent / "gui" / "src" / "App.jsx").read_text(
            encoding="utf-8"
        )
