...
核对日期：2026-09-10。当前实现已参考 CPA 改进独立网关的调度、会话和计量，没有接入或替换为外部代理引擎，也没有运行收费模型对照实验。[当前实现](gateway-design.md)
...
例如 A、B 两个 OAuth 账号都支持模型 M，顺序策略优先 A；A 返回允许换号的限流错误时才可能尝试 B。同名 Provider 共享模型也可在已加入池的候选中安全回退，但不跨 OAuth/Provider 身份类别，也不改变模型和推理强度。

当前实现已增加会话哈希与有界持久绑定、按来源在途负载选择。会话绑定最多 2048 条、闲置一小时；响应 ID 绑定、冷却和游标仍在进程内。全局上游并发上限为 8，没有分布式调度或账号级硬并发限制。[完整边界](gateway-design.md)
