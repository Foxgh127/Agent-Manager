...
import frozen_dll_isolation as isolation
...
class FrozenDllIsolationTests(unittest.TestCase):
    ...
    def test_build_uses_startup_hook_and_keeps_single_executable_mode(self):
        script = (Path(__file__).parent / 'build_exe.ps1').read_text(encoding='utf-8')
        self.assertIn('--onefile', script)
        self.assertIn('--runtime-hook (Join-Path $projectRoot "pyi_rth_agent_manager_dlls.py")', script)
        self.assertNotIn('--disable-windowed-traceback', script)
