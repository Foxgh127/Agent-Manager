...
export default function AppUpdatePanel({ api, notify, refreshKey = 0 }) {
  ...
  const check = useCallback(async () => {
    ...
    try {
      const initial = await api("/api/app-update");
      accept(initial.status);
      if (initial.status?.configured && initial.status?.download?.state !== "downloading" && !installStarted.current) {
  ...
  useEffect(() => { mounted.current = true; check(); return () => { mounted.current = false; }; }, [check, refreshKey]);
  ...
  return <article className="update-component manager-update" aria-label="Agent Manager 更新">
    ...
    {downloading || busy ? <span className="status-pill"><Loader2 size={14} className="spin" />{downloading ? `${progress}%` : busy === "install" ? "重启中" : "处理中"}</span>
      ...
      : <button className="button secondary compact" onClick={check} disabled={!status?.configured}>{status?.state === "current" ? <Check size={14} /> : <RefreshCw size={14} />}{status?.state === "current" ? "已是最新版" : "检查更新"}</button>}
