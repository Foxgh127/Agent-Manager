    ...
    try {
      await run("start", async () => {
        const result = await api("/api/relay-login/start", {
          method: "POST",
          body: JSON.stringify({ url: portalUrl.trim(), accountId: relayAccountId || undefined }),
          timeoutMs: 45_000,
        });
        selectedPreview.current = "";
        setSelected(new Set());
...
function RelayReloginModal({ account, groupId, syncToApi, notify, onDone, onClose }) {
  return (
    <Modal
      title={`重新登录 ${account.name || account.siteName || "中转站账号"}`}
      description="完成网页登录后自动继续刷新，无需重新选择或导入 API Key"
      onClose={onClose}
      wide
    >
      <div className="modal-body">
        <RelayLoginPanel
          ...
          initialPortalUrl={account.portalUrl || account.origin || ""}
...
function ProductHome({ onOpenCodex, onOpenClaude }) {
  return (
    <main id="main-content" className="product-home">
      ...
      <footer className="home-footer">Agent Manager 9.13.0 · 本地优先 · 凭据加密保存</footer>
    </main>
  );
}
