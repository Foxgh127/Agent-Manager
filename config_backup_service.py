...
Manual and recovery originals never expire. Inspection itself is read-only.
"""
...
def _references() -> tuple[set[str], set[str], bool]:
    ...
    uncertain = False
    sources = {core.RUNTIME_OVERLAY_FILE}
...
def prune_automatic(*, keep_paths=()) -> dict:
    ...
    with LOCK:
        ...
        rows = [row for row in list_backups() if row["kind"] == "auto"]
...
def create_backup(raw: bytes, *, kind: str) -> dict:
    ...
    with LOCK:
        if kind == "auto":
            digest = fingerprint(raw)
            automatic = [row for row in list_backups() if row["kind"] == "auto"]
