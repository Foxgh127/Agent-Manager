...
class AppUpdateService:
    def __init__(self, current_version, config_path, download_dir, bundled_source_path=None,
                 cache_path=None, *, platform="windows-x64", fetcher=None, install_supported=False, release_epoch=0):
        ...
        self.release_epoch = max(0, int(release_epoch))
    ...
    def _source_fingerprint(self, source):
        return _fingerprint({"source": source, "releaseEpoch": self.release_epoch}) if self.release_epoch else _fingerprint(source)

    def _require_release_epoch(self, manifest, version):
        if not self.release_epoch:
            return
        if (not isinstance(manifest, dict) or manifest.get("appId") != APP_ID
                or manifest.get("releaseEpoch") != self.release_epoch
                or str(manifest.get("version") or "") != version):
            raise UpdateError("该发布不属于当前版本系列；请从项目的 1.0.0 及后续发布中获取更新。", "release_epoch_mismatch")
    ...
    def status(self):
        ...
        with self._lock:
            matches = source is not None and self._source_fingerprint(source) == self._source_id
    ...
    def _discover(self, source):
        ...
        for _, release in sorted(candidates, key=lambda pair: pair[0], reverse=True):
            try:
                return self._github_asset_release(release, source)
            except UpdateError as exc:
                if self.release_epoch and exc.code in {"release_epoch_mismatch", "release_epoch_missing"}:
                    last_epoch_error = exc
                    continue
                raise
    ...
    def check(self):
        ...
        try:
            ...
            source_id = self._source_fingerprint(source)
