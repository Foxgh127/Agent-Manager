...
class _PrefixedResponse:
    ...
    def __init__(self, response: Any, prefix: bytes):
        ...
        self._gateway_identity_fingerprint = str(getattr(response, "_gateway_identity_fingerprint", ""))
        self.closed = False
    ...
    for item in items:
        ...
        if kind == "message":
            ...
            if isinstance(output, list) and any(
                ...
                # Tool outputs can contain multimodal content too; server
                # file references do not establish a portable full history.
                return None
            assistant_seen = True
        else:
...
def _abort_upstream_response(response: Any) -> None:
    """Close the actual transport before waiting on buffered-reader cleanup.

    urllib may have already called socket.close() while makefile() still owns
    the descriptor. A second close() only repeats that deferred close, and on
    Windows shutdown alone need not wake an in-flight timed recv. Detach the
    transport and close its handle so the reader unwinds before response.close
    tries to acquire the BufferedReader lock held by that same reader.
    """
    pending = [(response, 0)]
    seen = set()
    while pending:
        ...
        if isinstance(candidate, socket.socket):
            try:
                descriptor = candidate.detach()
                if descriptor != -1:
                    # socket.close handles Windows SOCKETs; os.close does not.
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/v1/responses" and self._header_values("Upgrade"):
                from agent_manager.gateway.websocket import serve_responses_websocket
