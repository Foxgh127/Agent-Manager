...
class DashboardOriginChanged(core.ManagerError):
    """A www/apex navigation needs an explicit new connection, not reauth."""

    def __init__(self, details: dict) -> None:
        self.details = dict(details)
        super().__init__(
            f"网页登录已跳转到 {details['currentOrigin']}，与保存的网站地址 {details['savedOrigin']} 不同。"
            "请使用当前网站地址重新连接，并在新窗口重新登录。"
        )


def _browser_origin_mismatch(expected_origin: str, current_url: str) -> dict | None:
    """Describe exact HTTPS www/apex counterparts without trusting the alias."""
    try:
        expected = urllib.parse.urlsplit(expected_origin)
        current = urllib.parse.urlsplit(current_url)
        if (expected.scheme != "https" or current.scheme != "https"
                or (expected.port or 443) != 443 or (current.port or 443) != 443
                or current.username or current.password):
            return None
        saved_host = (expected.hostname or "").casefold().rstrip(".")
        current_host = (current.hostname or "").casefold().rstrip(".")
        if not saved_host or not current_host or not (
            saved_host == "www." + current_host or current_host == "www." + saved_host
        ):
            return None
        # Temporary OAuth callbacks can still return to the original origin.
        if re.search(r"(?:^|/)(?:oauth2?|authorize|callback|sso)(?:/|$)", current.path, re.I):
            return None
        query_names = {name.casefold() for name, _value in urllib.parse.parse_qsl(current.query)}
        if query_names.intersection({"code", "state", "access_token", "id_token"}):
            return None
        return {"savedOrigin": "https://" + saved_host,
                "currentOrigin": "https://" + current_host,
                "reconnectUrl": "https://" + current_host,
...
def _serialized_dashboard(function):
    ...
    def invoke(self, account_id, *args, **kwargs):
        ...
        self._release_retained_for_other_account(account_key)
...
def _is_browser_challenge_cookie(name: str) -> bool:
    return name.casefold() in {"cf_clearance", "__cf_bm", "__cfseq"} or name.casefold().startswith(("cf_chl_", "__cf_chl_"))
...
def _window_cookie_records(window: object, expected_origin: str) -> list[dict]:
    ...
    for container in containers if isinstance(containers, (list, tuple)) else [containers]:
        ...
        for candidate in candidates:
            ...
            if not name or not value or not host or _is_browser_challenge_cookie(name):
                continue
...
def _cookie_header(session: dict, url: str) -> str:
    ...
    for item in session.get("cookies", []) if isinstance(session.get("cookies"), list) else []:
        ...
        if not name or not value or _is_browser_challenge_cookie(name) or (domain != host and not host.endswith(f".{domain}")):
            continue
...
def _merge_response_cookies(session: dict, url: str, headers: object) -> None:
    ...
    with COOKIE_LOCK:
        current = { (str(row.get("name")), str(row.get("domain") or host).lstrip(".").casefold(), str(row.get("path") or "/")): dict(row)
                    ...
                    and not _is_browser_challenge_cookie(str(row.get("name") or "")) }
        for value in values[:80]:
            ...
            for name, cookie in jar.items():
                if _is_browser_challenge_cookie(name):
                    continue
...
class RelayPortalService:
    ...
    def _expired_locked(self) -> bool:
        # The TTL bounds interactive login attempts. A verified retained page
        # is instead authenticated afresh on every explicit balance read.
        if self.window is not None and self._session.get("_retainedBalanceIdentity"):
            return False
    ...
    def _report_origin_mismatch(self, session_id: object, details: dict) -> dict:
        with self.lock:
            self._require_current_session_locked(session_id)
            self._session.update({
                "status": "origin_mismatch", "message": str(DashboardOriginChanged(details)),
                "currentUrl": details["currentOrigin"], "originMismatch": dict(details),
                "error": None, "updatedAt": core.now_iso(), "_autoAuthInFlight": False,
            })
            self._session["autoAuth"].update({"enabled": False, "state": "origin_mismatch", "nextCheckInMs": 0})
    ...
    def _validate_session(self, session_id: object) -> tuple[object, str, str]:
        ...
        if self._origin(current_url) != expected_origin:
            mismatch = _browser_origin_mismatch(expected_origin, current_url)
            if mismatch:
                self._report_origin_mismatch(session_id, mismatch)
                raise DashboardOriginChanged(mismatch)
        ...
        try:
            ...
            def closed() -> None:
                with self.lock:
                    if self._session.get("sessionId") == session_id:
                        self.window = None
                        self._session.pop("_retainedBalanceIdentity", None)
    ...
    def check_auto_auth(self, session_id: object, *, force: bool = False) -> dict:
        ...
        try:
            ...
            if self._origin(current_url) != expected_origin:
                mismatch = _browser_origin_mismatch(expected_origin, current_url)
                if mismatch:
                    return self._report_origin_mismatch(supplied, mismatch)
                return self._auto_auth_waiting(
                    supplied,
                    cancel_event,
                    "等待第三方授权完成并返回中转站…",
            ...
            self._retain_window_for_balance(supplied, full_identity, preview["adapter"])
            return self.public_state()
    ...
    def _release_retained_for_other_account(self, account_id: str) -> None:
        with self.lock:
            binding = self._session.get("_retainedBalanceIdentity")
            session_id = self._session.get("sessionId")
        if isinstance(binding, dict) and binding.get("accountId") != account_id:
            self._release_window(session_id=session_id)

    def _retain_window_for_balance(self, session_id: str, identity: dict, adapter: str) -> None:
        """Hide one successfully verified page, or release it if hiding fails."""
        try:
            window, _portal_url, origin = self._validate_session(session_id)
            with self.lock:
                event = self._session.get("_cancelEvent")
                if self._session.get("status") != "reauthenticated" or (
                    isinstance(event, threading.Event) and event.is_set()
                ):
                    raise core.ManagerError("网页登录已取消，未保留后台窗口。")
                account_id = str(self._session.get("accountId") or "")
                if not account_id or not (identity.get("id") or identity.get("email")):
                    raise core.ManagerError("缺少已验证的账号身份，未保留后台窗口。")
            window.hide()
            with self.lock:
                self._require_current_session_locked(session_id)
                if self.window is not window or (isinstance(event, threading.Event) and event.is_set()):
                    raise core.ManagerError("网页登录窗口已关闭，未保留后台窗口。")
                self._session["_retainedBalanceIdentity"] = {
                    "accountId": account_id, "origin": origin, "adapter": adapter,
                    "user": dict(identity),
                }
                self._session["autoAuth"].update({"enabled": False, "nextCheckInMs": 0})
        except Exception:
            # Old webview implementations without hide() retain no background
            # window. A completed login remains successful in either case.
            self._release_window(session_id=session_id)
    ...
    def _probe_open_dashboard_quick(self, account: dict) -> dict | None:
        ...
        try:
            if isinstance(binding, dict):
                ...
                if binding.get("accountId") != str(account.get("id") or "") or core._provider_url_origin(binding.get("origin")) != origin:
                    return None
                _assert_dashboard_identity(
                    binding["user"], account.get("user") or {},
                    expected_adapter=binding["adapter"], actual_adapter=str(account.get("adapter") or ""),
                )
            result = self._read_open_dashboard_quick(account)
            return result
        finally:
            if isinstance(binding, dict) and result is None:
                self._release_window(session_id=session_id)

    def _read_open_dashboard_quick(self, account: dict) -> dict | None:
        ...
        for _ in range(2):
            ...
            with self.lock:
                ...
                if isinstance(cancel_event, threading.Event) and cancel_event.is_set():
                    return None
            if not isinstance(raw, dict) or raw.get("adapter") != adapter or not _is_user_response(raw.get("user") or {}):
                return None
    ...
    def refresh_account(self, account_id: object, *, full: bool = False) -> dict:
        ...
        try:
            ...
            try:
                ...
                if isinstance(browser_exc, DashboardOriginChanged):
                    origin_mismatch = browser_exc.details
            ...
            if origin_mismatch:
                result["originMismatch"] = dict(origin_mismatch)
            return result
