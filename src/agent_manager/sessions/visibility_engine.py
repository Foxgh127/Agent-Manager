...
def _row_may_need_quick_scan(row: Mapping[str, Any], columns: set[str], provider: str) -> bool:
    if row.get("_catalogProviderMismatch") or row.get("_catalogEntryMissing"):
        return True
...
def _inspect_catalog(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {"status": "supported", "hostId": host_id, "remoteIds": sorted(remote_ids),
            "nativeRebuild": _inspect_native_catalog_rebuild(connection, host_id),
            "rows": {str(item["thread_id"]): item for item in rows if item["host_id"] == host_id}}


def _inspect_native_catalog_rebuild(connection: sqlite3.Connection, host_id: str) -> dict[str, Any]:
    """Recognize the desktop's existing full-scan request, without inventing rows.

    Codex Desktop 26.901.6511.0 requestStartupSync() requests a full native
    thread/list scan when lastFullReconciliationAt is null. Native scanning
    owns title/recency/sequence/revision and any subsequent catalog upserts.
    A saved checkpoint or unrecognized schema must not be overwritten.
    """
    schemas = {
        "local_thread_catalog_sync_state": [
            ("host_id", "TEXT", 0, None, 1), ("watermark_updated_at", "REAL", 0, None, 0),
            ("initial_build_complete", "INTEGER", 1, "0", 0),
            ("observation_sequence", "INTEGER", 1, "0", 0),
            ("last_full_reconciled_at", "INTEGER", 0, None, 0)],
        "local_thread_catalog_scan_checkpoints": [
            ("host_id", "TEXT", 0, None, 1), ("checkpoint", "TEXT", 1, None, 0),
            ("failed_at", "INTEGER", 0, None, 0)],
        "local_thread_catalog_scan_entries": [
            ("host_id", "TEXT", 1, None, 1), ("thread_id", "TEXT", 1, None, 2),
            ("removed", "INTEGER", 1, "0", 0)],
    }
    for table, expected in schemas.items():
        actual = [tuple(row)[1:] for row in connection.execute(f"PRAGMA table_info({_quote_identifier(table)})")]
        if (actual != expected or connection.execute(
                "SELECT 1 FROM sqlite_master WHERE type='trigger' AND tbl_name=?", (table,)).fetchone()
                or connection.execute(f"PRAGMA foreign_key_list({_quote_identifier(table)})").fetchone()):
            return {"status": "unsupported_coordination_schema"}
    for table in ("local_thread_catalog_scan_checkpoints", "local_thread_catalog_scan_entries"):
        if connection.execute(f"SELECT 1 FROM {_quote_identifier(table)} WHERE host_id=? LIMIT 1", (host_id,)).fetchone():
            return {"status": "scan_in_progress"}
    row = connection.execute("SELECT * FROM local_thread_catalog_sync_state WHERE host_id=?", (host_id,)).fetchone()
    if row is None:
        return {"status": "missing_sync_state"}
    values = dict(row)
    if (values["initial_build_complete"] not in (0, 1)
            or not isinstance(values["observation_sequence"], int) or values["observation_sequence"] < 0
            or (values["last_full_reconciled_at"] is not None
                and (not isinstance(values["last_full_reconciled_at"], int) or values["last_full_reconciled_at"] < 0))):
        return {"status": "invalid_sync_state"}
    return {"status": "pending" if values["last_full_reconciled_at"] is None else "available",
            "previousReconciledAt": values["last_full_reconciled_at"]}
...
def inspect_session_visibility(
    ...
    *,
    ...
    for database in selected_databases:
        ...
        if catalog.get("status") == "supported":
            for row in database["rows"]:
                ...
                row["_catalogEntryMissing"] = cached is None
    ...
    catalog_rebuild_paths: set[str] = set()
    ...
    for database in selected_databases:
        ...
        for row in database["rows"]:
            ...
            rebuild = catalog.get("nativeRebuild", {})
            rebuild_path = catalog.get("path", database["path"])
            request_rebuild = (cached is None and thread_id in catalog_missing_ids
                               and rebuild.get("status") == "available"
                               and rebuild_path == DESKTOP_CATALOG_PATH
                               and rebuild_path not in catalog_rebuild_paths)
            needs_write = bool(updates or request_rebuild or rollout.get("provider") != provider
            ...
            if request_rebuild:
                catalog_rebuild_paths.add(rebuild_path)
                actions.append({
                    "kind": "sqlite_update", "table": "local_thread_catalog_sync_state",
                    "database": rebuild_path, "threadId": thread_id, "idColumn": "host_id", "hostId": catalog["hostId"],
                    "expected": {"last_full_reconciled_at": rebuild["previousReconciledAt"]},
                    "set": {"last_full_reconciled_at": None}, "reasons": ["catalog_native_rebuild_requested"],
                    "archived": False,
                    "rolloutEvidence": {"path": rollout["path"], "fingerprint": rollout["fingerprint"],
                                        "lineSha256": rollout["sessionMetaLineSha256"]},
                })
    ...
    report: dict[str, Any] = {
        ...
        "completion": {
            ...
            "catalogRecovery": ("native_rebuild_available" if catalog_missing_ids and catalog_rebuild_paths
                                else "native_rebuild_pending" if catalog_missing_ids and desktop_catalog.get("nativeRebuild", {}).get("status") == "pending"
...
def _sqlite_target(action: Mapping[str, Any]) -> tuple[str, str, list[Any]]:
    ...
    if (table == "local_thread_catalog_sync_state" and action.get("idColumn") == "host_id"
            and action.get("database") == DESKTOP_CATALOG_PATH
            and isinstance(action.get("hostId"), str) and action["hostId"]
            and set(action.get("set", {})) == {"last_full_reconciled_at"}
            and set(action.get("expected", {})) == {"last_full_reconciled_at"}
            and action["set"]["last_full_reconciled_at"] is None):
        return table, '"host_id" = ?', [action["hostId"]]
    ...
    for relative, database_actions in sorted(grouped.items()):
        ...
        try:
            ...
            for action in database_actions:
                ...
                if table == "local_thread_catalog_sync_state":
                    checked = _inspect_catalog(connection)
                    if (checked.get("hostId") != action["hostId"]
                            or checked.get("nativeRebuild", {}).get("status") != "available"):
                        raise SessionVisibilityError("目录重建请求已变化或存在进行中的扫描，未覆盖原检查点。")
