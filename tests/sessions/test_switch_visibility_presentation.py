...
def test_reenabling_backfill_is_lazy_without_file_or_thread_work():
    with patch.object(agent_manager.gateway.service.threading, "Thread") as thread, patch.object(agent_manager.gateway.service, "codex_session_usage_backfill_step") as step:
...
def test_internal_usage_snapshot_never_starts_background_history_work():
    manager = agent_manager.gateway.service.Web2APIManager.__new__(agent_manager.gateway.service.Web2APIManager)
    manager.usage_stats = SimpleNamespace(snapshot=lambda: {})
    manager.last_usage_error = None
    with patch.object(agent_manager.gateway.service, "codex_session_usage_snapshot", return_value={"coverage":{"backfill":{"pendingFiles":34}}}), patch.object(agent_manager.gateway.service, "account_attribution_snapshot", return_value={}), patch.object(agent_manager.gateway.service, "request_codex_session_usage_backfill") as start:
