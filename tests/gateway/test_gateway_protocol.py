...
import agent_manager.gateway.websocket as ws
...
def test_websocket_disconnect_interrupts_pending_upstream_read(backend, monkeypatch, prefixed, attempt):
    del attempt  # Repeat the real socket cancellation to exercise scheduling.
    if prefixed:
        opened = core._open_same_origin_request
        monkeypatch.setattr(core, "_open_same_origin_request", lambda *args, **kwargs:
                            gateway._PrefixedResponse(opened(*args, **kwargs), b""))
    existing_workers = set(threading.enumerate())
    ...
    # The upstream flush alone precedes the proxy's response-header/abort-hook
    # registration. Observe a forwarded event before testing a pending read.
    opcode, frame = client.receive()
    assert opcode == 1 and json.loads(frame)["type"] == "response.created"
    workers = [thread for thread in threading.enumerate()
               if thread not in existing_workers and thread.name == "web2api-ws-turn"]
    ...
    if not acquired:
        ...
        debug = Path("artifacts/gateway-cancellation")
        debug.mkdir(parents=True, exist_ok=True)
        with (debug / "blocked-threads.log").open("w") as log:
            faulthandler.dump_traceback(file=log, all_threads=True)
    assert acquired
    backend["server"]._upstream_slots.release()
    for worker in workers:
        worker.join(timeout=1)
        assert not worker.is_alive(), "Cancellation left the generation worker running"
    assert backend["manager"]._lookup_response_binding("paused") is None
