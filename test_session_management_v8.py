...
class SessionManagementV8Tests(unittest.TestCase):
    ...
    def test_manager_pins_persist_without_unsupported_git_metadata_calls(self):
        ...
        self.assertEqual(session_preferences.pinned_ids(core.load_settings()), {"thread-one"})
        rows = session_preferences.apply_to_threads([{"id": "thread-one"}, {"id": "thread-two", "pinned": True}])
